...
def test_kettle_prefilled_from_message_skips_redundant_description_asks_name():
    """Kettle ifadesi → çay/kahve tedarik satırı (hk_coffee_tea_supplies); önce adet (detail_int), genel «Diğer» metni yok."""
    orch, _, _ = build_orchestrator()
    ...
    assert res.meta.action.step == "detail_int"
    low = res.message.lower()
    assert "neye ihtiyaç duyduğunuzu" not in low
    assert "«diğer»" not in low and "diger»" not in low
    assert "kahve" in low or "çay" in low or "cay" in low or "adet" in low
...
def test_su_isitici_eksik_opens_kettle_request_form():
    ...
    for i, msg in enumerate(("su ısıtıcısı eksik", "su isiticisi eksik")):
        ...
        assert res.meta.action.step == "detail_int", msg
        assert "kahve" in low or "çay" in low or "cay" in low or "adet" in low, msg
...
def test_request_and_complaint_cases():
    ...
    assert cmp_.meta.action and cmp_.meta.action.kind == "chat_form" and cmp_.meta.action.operation == "complaint"
    low = cmp_.message.lower()
    assert (
        "şikayet" in low
        or "kategori" in low
        or "1." in low
...
def test_temizlik_kotu_still_complaint():
    ...
    assert res.meta.action and res.meta.action.kind == "chat_form" and res.meta.action.operation == "complaint"
    assert res.type == "inform"
...
def test_odama_hamburger_opens_room_service_module_not_request_form():
    orch, rag, _ = build_orchestrator()
    for msg in (
        "odama hamburger istiyorum",
        "odaya hamburger istiyorum",
        "odama hamburge istiyorum",
    ):
        rag.called = False
        res = orch.handle(ChatRequest(message=msg, ui_language="tr", locale="tr"))
        assert res.meta.intent == "hotel_info", msg
        assert rag.called is False, msg
        assert res.meta.action and res.meta.action.kind == "open_room_service_module", msg


def test_pizza_order_opens_room_service_module_not_request_form():
    orch, rag, _ = build_orchestrator()
    for msg in ("pizza istiyorum", "pizza sitiyorum", "pizza sityourm"):
        rag.called = False
        res = orch.handle(ChatRequest(message=msg, ui_language="tr", locale="tr"))
        assert res.meta.intent == "hotel_info", msg
        assert rag.called is False, msg
...
def test_room_service_multilingual_room_plus_dish_opens_module():
    orch, rag, _ = build_orchestrator()
    cases = (
        ("pour ma chambre une pizza s'il vous plaît", "en", "en"),
        ("auf mein zimmer schnitzel bestellen", "de", "de"),
        ("dostawa do pokoju pierogi", "pl", "pl"),
    )
    for msg, ui, loc in cases:
        rag.called = False
        res = orch.handle(ChatRequest(message=msg, ui_language=ui, locale=loc))
        assert res.meta.intent == "hotel_info", msg
        assert rag.called is False, msg
        assert res.meta.action and res.meta.action.kind == "open_room_service_module", msg


def test_odama_beverage_opens_room_service_module():
    orch, rag, _ = build_orchestrator()
    for msg in (
        "odama cola istiyorum",
        "odaya meyve suyu istiyorum",
        "odama içecek istiyorum",
        "cola istiyorum",
        "meyve suyu lütfen",
    ):
        rag.called = False
...
def test_oda_servisi_calismiyor_does_not_open_room_service_module():
    """Arıza/şikâyet dili varken «oda servisi» alt dizgisi sipariş modülünü açmasın."""
    orch, rag, _ = build_orchestrator()
    res = orch.handle(ChatRequest(message="oda servisi çalışmıyor", ui_language="tr", locale="tr"))
    assert not (res.meta.action and res.meta.action.kind == "open_room_service_module")
...
def test_routing_guest_relations_contact_and_generic_problem():
    ...
    assert prob.meta.action and prob.meta.action.kind == "chat_form" and prob.meta.action.operation == "complaint"
    plow = prob.message.lower()
    assert "kategori" in plow or "1." in plow
    assert intent.calls == 0
...
def test_complaint_bare_word_and_discomfort_open_form():
    ...
    for r in (tr_sik, tr_rah, en_unc):
        ...
        assert r.meta.action and r.meta.action.kind == "chat_form" and r.meta.action.operation == "complaint"
    assert intent.calls == 0
...
def test_resepsiyon_room_device_fault_routes_fault_not_reception_contact():
    """Ön büro kelimesi + oda donanımı arızası → arıza formu; yalnız resepsiyon iletişim formu değil."""
    orch, _, _ = build_orchestrator()
    res = orch.handle(
        ChatRequest(
            message="resepsiyon klima çalışmıyor",
            ui_language="tr",
            locale="tr",
            user_id="u-rec-fault",
            session_id="s-rec-fault",
        )
    )
    assert res.meta.intent == "fault_report"
    assert res.meta.action and res.meta.action.kind == "chat_form"
...
def test_on_buro_phrase_routes_reception_request():
    orch, _, _ = build_orchestrator()
    res = orch.handle(
        ChatRequest(
            message="ön büro iletişim istiyorum",
            ui_language="tr",
            locale="tr",
            user_id="u-onburo",
            session_id="s-onburo",
    ...
    assert res.meta.intent == "request"
    assert res.meta.action and res.meta.action.kind in ("create_guest_request", "chat_form")


def test_resepsiyon_where_routes_hotel_info_not_reception_redirect():
    orch, rag, _ = build_orchestrator()
    res = orch.handle(
        ChatRequest(
            message="resepsiyon nerede",
            ui_language="tr",
            locale="tr",
            user_id="u-rec-nerede",
            session_id="s-rec-nerede",
        )
    )
    assert res.meta.intent == "hotel_info"
    assert res.meta.action is None or res.meta.action.kind != "create_guest_request"


def test_oda_temizligi_memnun_kalmadim_routes_complaint_not_housekeeping():
    orch, _, _ = build_orchestrator()
    res = orch.handle(
        ChatRequest(
            message="oda temizliği memnun kalmadım",
            ui_language="tr",
            locale="tr",
            user_id="u-hk-diss",
            session_id="s-hk-diss",
        )
    )
    assert res.meta.intent == "complaint"
    assert res.meta.action and res.meta.action.kind == "chat_form" and res.meta.action.operation == "complaint"
...
def test_full_operational_routing_matrix():
    ...
    for phrase in ("komşu çok gürültülü şikayet", "oda pis şikayetçiyim"):
        ...
        assert r.meta.action and r.meta.action.kind == "chat_form" and r.meta.action.operation == "complaint", phrase
        assert intent.calls == 0
    ...
    r, _, intent = run("odada teknik arıza")
...
def test_request_form_detail_enum_allows_switch_before_valid_choice():
    """Kategori adımındayken (ör. havlu talebi) başka tedarik konusuna geçilebilir; adet (detail_int) kilitlenir."""
    orch, _, _ = build_orchestrator()
    ...
    low = r2.message.lower()
    assert "battaniye" in low or "blanket" in low or "yorgan" in low or "duvet" in low
    assert "havlu" not in low and "handtuch" not in low
...
def test_request_form_locks_after_valid_enum_choice_on_detail_int():
    """detail_int adımında sayı yerine serbest metin → aynı adımda kalınır; geçerli sayı beklenir."""
    orch, rag, _ = build_orchestrator()
    uid, sid = "u-lock-post", "s-lock-post"
    r1 = orch.handle(
        ChatRequest(message="ek havlu lazım", ui_language="tr", locale="tr", user_id=uid, session_id=sid)
    )
...
def test_second_tamam_after_cancel_ack_not_cancel_ack_again():
    ...
    assert "başka bir konuda" in low1 and ("yardımcı" in low1 or "eşlik" in low1)
    r2 = orch.handle(ChatRequest(message="tamam", ui_language="tr", locale="tr", user_id=uid, session_id=sid))
    low2 = r2.message.lower()
    assert not ("başka bir konuda" in low2 and ("yardımcı" in low2 or "eşlik" in low2))
...
def test_bare_sikayet_opens_complaint_form_button():
    ...
    assert res.meta.action and res.meta.action.kind == "chat_form" and res.meta.action.operation == "complaint"
    low = res.message.lower()
    assert "kategori" in low or "1." in low
...
def test_lost_found_policy_question_still_complaint_form_with_info():
    """Kayıp eşya saklama süresi saf bilgi sorusu → politika metni; sohbet formu açılmaz."""
    orch, _, _ = build_orchestrator()
...
def test_request_form_category_step_bornoz_keeps_form_not_hotel_info():
    ...
    low = r.message.lower()
    assert "bornoz" in low or "bademantel" in low or "diğer" in low or "diger" in low or "açıklama" in low or "aciklama" in low
    assert rag.called is False
...
def test_full_name_ack_prompts_four_locales_consistency():
    ...
    assert "su ısıtıcı" in tr.lower() or "kettle" in tr.lower() or "isitici" in tr.lower()
