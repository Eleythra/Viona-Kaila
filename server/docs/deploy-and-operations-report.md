...
| **WhatsApp Cloud API** | Operasyon: 3 onaylı şablon (`viona_issue_notification` / `viona_request_notification` / `viona_complaint_notification`) + misafir ilişkileri şablonu (bildirim + geç çıkış) + arıza → TECH; istek → HK; şikayet / bildirim / geç çıkış → Front (virgülle çoklu numara, her numaraya ayrı `POST …/messages`). İsteğe bağlı Meta «Dynamic URL» / «Panelde Aç»: `WHATSAPP_HK_PANEL_URL_BUTTON`, `WHATSAPP_TECH_PANEL_URL_BUTTON`, `WHATSAPP_FRONT_PANEL_URL_BUTTON` (=1) |
...
## 4a. WhatsApp operasyon (Cloud API)

Kalıcı **access token**, **Phone number ID**, Meta’da onaylı **şablonlar** ve `WHATSAPP_TECH_RECIPIENTS` / `WHATSAPP_HK_RECIPIENTS` / `WHATSAPP_FRONT_RECIPIENTS` listeleri `server/.env.example` içinde açıklanır. Gönderim log öneki: `[whatsapp_ops]`.
...
| `type: guest_notification` / `late_checkout` | `WHATSAPP_FRONT_RECIPIENTS` (ön büro; şikayet ile aynı liste) |
