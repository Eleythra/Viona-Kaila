...
| **Node API** (`server/`, canlıda örn. `https://api.eleythra.com`) | Express: misafir formları, admin API, PDF, chat proxy |
...
API kökü: `js/viona-backend-url.js` → `https://api.eleythra.com/api` (Node VPS; Python asistan ayrı `ASSISTANT_CHAT_ENDPOINT`).
